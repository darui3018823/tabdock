...
function saveLoginState(user) {
    ...
    if (window.subscriptionManager && typeof window.subscriptionManager.scheduleNotifications === 'function') {
        console.log('ログイン成功に伴い、サブスクリプション通知を再スケジュールします。');
        window.subscriptionManager.scheduleNotifications({ reschedule: true, immediate: true })
            .catch(err => console.error('通知の再スケジュールに失敗しました:', err));
    }
}
